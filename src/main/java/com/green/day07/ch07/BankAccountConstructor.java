...
class BankAccount2 {
    ...
    정의 : 메소드와 다른점 2가지
    1. 이름이 클래스명과 같다.
    2. 반환형 (리턴타입)을 작성하지 않는다. - void 아님

    생성자 특징은 오로지 객체 생성할때만 호출할 수 있다. 객체생성 이후에는 호출 불가

    생성자를 작성하지 않으면 기본생성자가 자동으로 만들어진다.
    하지만 생성자를 하나라도 작성을 하게 되면 기본 생성자를 자동으로 만들어주지 않는다.
    그럴때는 기본 생성자를 만들어줘야 한다.

    오버로딩 (overloading) : 같은 이름의 메소드를 여러개 만들 수 있는 기법 , 파라미터가 다르면 가능
    ex) BankAccount2(String a)
    ex) BankAccount2(String b)
        BankAccount2(int a) , 타입의 종류 || 이름 등등 다르면 가능함
     */
    ...
    BankAccount2(String acc, String ss , int bal){ // 파라미터가 있는 생성자 or 오버로딩된 생성자
        accNumber = acc;
        ssNumber = ss;
        balance = bal;
    }

    void initAccount(String acc, String ss , int bal){ // 이건 initAccount 메소드
        accNumber = acc;
        ssNumber = ss;
        balance = bal;
    }
